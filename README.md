...
Rather than relying on third-party packages, this setup leverages the strict, uncompromising architecture of OpenBSD's native daemons.
...
During installation, configure the WAN interface for autoconfiguration (IPv4/IPv6) and assign a static IPv4 address to the LAN interface (using 192.168.1.1 for this example). Set the system hostname and domain name. We will use ```home.arpa``` as the domain throughout this guide.

## 2. Temporarily disable daemons (This guide assumes the machine is connected to the internet.)
...
# --- Tables ---
# Martian (unroutable/private) addresses - IPv4
table <martians> {
...
# Martian (unroutable/special-purpose) addresses - IPv6
table <martians6> {
...
# Normalization: Clean incoming packets. 
match in all scrub (no-df random-id)

# MSS Clamping: Apply only on the egress interface to handle WAN encapsulation overhead.
match out on egress scrub (max-mss 1440)

# Outbound NAT for internet access (IPv4 only)
...
# Block martian addresses (IPv4/IPv6)
...
# Allow general LAN traffic in (Preserves 'last match wins' for future internal rules)
pass in on $lan

# ICMP diagnostics strictly bound to WAN ingress
# IPv4 is safely bound to the unicast interface IP
pass in quick on egress inet proto icmp from any to (egress) icmp-type { echoreq, unreach }

# IPv6 restricted to the firewall's IPs AND link-local multicast
# This permits ISP gateway routing (NDP/RA) but protects internal LAN global IPs
pass in quick on egress inet6 proto ipv6-icmp from any to { (egress), ff02::/16 } icmp6-type { \
    echoreq, echorep, unreach, toobig, timex, paramprob, \
    neighbrsol, neighbradv, routersol, routeradv \
}

# DHCPv6 client allow (Prefix Delegation)
# Strictly bound to the egress interface to prevent external probing of LAN port 546
...
* `pass in quick on egress inet proto icmp from any to (egress) icmp-type { echoreq, unreach }`: This rule explicitly binds incoming ICMP traffic to the `(egress)` IP address. This ensures that diagnostic pings (`echoreq`) and routing error messages (`unreach`) are only accepted when they are targeting the firewall's public IP, rather than attempting to pass through it.
* `pass in quick on egress inet6 proto ipv6-icmp from any to { (egress), ff02::/16 } icmp6-type { \
    echoreq, echorep, unreach, toobig, timex, paramprob, \
    neighbrsol, neighbradv, routersol, routeradv \
...
## Author
![Example Image](crimsonghost.jpg)
